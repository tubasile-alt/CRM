...
- **Condensed Consultation History** (November 2025): Completely redesigned history system with accordion UI showing consultations grouped by date. Implementation uses deterministic `appointment_id` foreign key in Note model for reliable grouping, with temporal fallback for legacy data. Full details expand on click including: all note sections (queixa, anamnese, diagnostico, conduta), cosmetic procedures with execution status and Brazilian currency formatting (R$ 1.500,00), hair transplant planning with Norwood classification, and indicated/performed procedures for both Patologia and Cosmiatria. Badge system differentiates finalized consultations from drafts. Zero data loss guarantee via dual-path grouping strategy. Template intelligently hides placeholder text "[Conduta registrada via procedimentos]" when displaying procedure-only consultations.
