console.log("prontuario.js carregado v=20260309-fixed-right-panel-1");
...
async function saveAttentionNote() {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (attentionBadge) attentionBadge.style.display = content ? "inline-block" : "none";
...
function scrollToConsultation(id) {
  ...
  setCosmeticContextFromConsultation(consultId, false);
...
function getRightPanelElements() {
  return {
    panel: document.getElementById("cosmeticRightColumn"),
    panelTitle: document.getElementById("cosmeticRightColumnTitle"),
    panelContent: document.getElementById("cosmeticRightColumnContent"),
    left: document.getElementById("prontuarioLeftColumn")
  };
}

function updateMainLayoutColumns(forceShow = false) {
  const { left, panel } = getRightPanelElements();
...
function setRightPanel(titleHtml, bodyHtml, mode) {
  const { panelTitle, panelContent } = getRightPanelElements();
  if (!panelContent) return;

  currentRightPanelMode = mode || null;
  if (panelTitle) panelTitle.innerHTML = titleHtml || "Painel lateral";
  panelContent.innerHTML = bodyHtml || '<div class="text-muted small">Nenhum conteúdo carregado.</div>';
  showRightPanel();
}
...
function renderTransplantRightPanelFromScreen() {
  ...
  setRightPanel(
    '<i class="bi bi-scissors me-2"></i>Planejamento Cirúrgico',
    `
      <div class="card border-0 bg-light">
        <div class="card-body small">
...
function renderHistoricalConsultationRightPanel(consultationId, category) {
  ...
  if (category === "cosmiatria") {
    setCosmeticContextFromConsultation(consultationId, true);
    return;
  }
  ...
  if (category === "transplante_capilar") {
    ...
    if (!wrapper) {
      setRightPanel(
        '<i class="bi bi-scissors me-2"></i>Planejamento Cirúrgico',
        `<div class="text-muted small">Nenhum planejamento cirúrgico encontrado nesta consulta.</div>`,
        "transplante"
      );
      return;
    }
...
function handleCategoryChange(event) {
  ...
  if (newCategory === "patologia" && !timerStartTime) {
...
function initCategoryFromCheckedRadio() {
  ...
  if (checked) currentCategory = checked.value;
  updateCategoryTexts();
  toggleCategoryTabs();
}
...
function setCosmeticContextFromConsultation(consultationId, rerender = true) {
...
function getProceduresForCurrentContext() {
  if (!activeCosmeticContext) return cosmeticProcedures.slice();
...
function renderCosmeticConduct() {
  const procsAll = Array.isArray(cosmeticProcedures) ? cosmeticProcedures : [];
  ...
  if (!procsAll.length) {
    setRightPanel(
      '<i class="bi bi-heart-pulse me-2"></i>Painel Cosmiatria',
      '<div class="text-muted small">Nenhum procedimento planejado.</div>',
      "cosmiatria"
    );
    return;
  }
  ...
  setRightPanel(
    '<i class="bi bi-heart-pulse me-2"></i>Planejamento e Execução',
    html,
    "cosmiatria"
  );
}
...
async function loadExistingPlans() {
  ...
  try {
      ...
      data.grouped_plans.forEach((group) => {
        group.procedures.forEach((plan) => {
          cosmeticProcedures.push({
            ...
            appointmentId:
              group.consultation_info?.appointment_id ||
    ...
    renderCosmeticProceduresBase();
    updateCosmeticTotal();

    if (currentCategory === "cosmiatria" || activeCosmeticContext) {
      renderCosmeticConduct();
    }
  } catch (error) {
...
function updatePlanValue(index, val) {
  ...
  updateCosmeticTotal();
  if (currentCategory === "cosmiatria" || activeCosmeticContext) renderCosmeticConduct();
}
...
function renderCosmeticProceduresBase() {
  ...
  if (groupedCosmeticPlans.length > 0) {
    groupedCosmeticPlans.forEach((group) => {
      const groupId = group.consultation_key || group.consultation_info?.appointment_id || Math.random().toString(36).slice(2);
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document.querySelectorAll(".accordion-button[data-consultation-key]").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      if (categoryText.includes("cosmiatria")) {
        setCosmeticContextFromConsultation(consultationKey, true);
      }
    });
  });
  ...
  if (surgeriesList && typeof window.loadSurgeries === "function") {
...
window.renderCosmeticProcedures = renderCosmeticProcedures;
window.renderCosmeticConduct = renderCosmeticConduct;
