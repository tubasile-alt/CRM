console.log("prontuario.js carregado v=20260306-layout-right-2");
...
function renderCosmeticProcedures() {
  ...
  if (groupedCosmeticPlans.length > 0) {
    groupedCosmeticPlans.forEach((group) => {
      const headerRow = tbody.insertRow();
      headerRow.className = "consultation-group-header";
      headerRow.id = `group-${group.consultation_key || group.appointment_id || Math.random().toString(36).slice(2)}`;
      headerRow.innerHTML = `
...
function renderCosmeticConduct() {
  ...
  html += `
    <div class="mt-3 p-3 bg-light rounded shadow-sm">
      <div class="fw-bold">
        Total Planejado: <span class="text-primary">R$ ${formatMoneyBRL(total)}</span>
      </div>
    </div>
  `;
